...
with sync_playwright() as pw:
 ...
 if wanted('1B-T01'):
  p=page(b);d=ae(p,"""
   ...
   let releaseX2,enteredX2=false;PablicusController.register('X2',async()=>()=>{enteredX2=true;await new Promise(r=>releaseX2=r)});await PablicusController.navigate({screen:'X2'});mounts=[];const aa=PablicusController.navigate({screen:'A'});while(!enteredX2)await wait();const bb=PablicusController.navigate({screen:'B'}),cc=PablicusController.navigate({screen:'C'});releaseX2();await Promise.all([aa,bb,cc]);return{first,second:{screen:PablicusController.state().screen,mounts}};
 ...
 if wanted('1B-T03'):
  p=page(b);d=ae(p,"""
   let cleaned=0,release,started=false,mounted=[];await PablicusController.sessionChanged('user-1');PablicusController.register('stable',async()=>{mounted.push('stable');return()=>{cleaned++}});await PablicusController.navigate({screen:'stable'});const before=PablicusController.state();await PablicusController.sessionChanged('user-1');const refresh={before:before.sessionGeneration,after:PablicusController.state().sessionGeneration,cleaned};
   PablicusController.register('slow',async({isCurrent})=>{started=true;await new Promise(r=>release=r);if(isCurrent())mounted.push('slow');return()=>{cleaned++}});const slow=PablicusController.navigate({screen:'slow'});while(!started)await new Promise(r=>setTimeout(r,0));const logout=PablicusController.sessionChanged(null);release();await Promise.all([slow,logout]);const afterLogout={mounted:[...mounted],cleaned,state:PablicusController.state()};await PablicusController.sessionChanged('user-2');return{refresh,afterLogout,final:PablicusController.state()};
  """);app=app_path.read_text(encoding='utf-8');contract="sessionChanged(session?.user?.id||null)" in app and "sessionChanged();" not in app;ok=d['refresh']['before']==d['refresh']['after'] and d['refresh']['cleaned']==0 and 'slow' not in d['afterLogout']['mounted'] and d['afterLogout']['cleaned']>=1 and d['final'].get('sessionUserId')=='user-2' and contract;record('1B-T03',ok,{'runtime':d,'auth_contract':contract});p.close()
 if wanted('1B-T04'):
  p=page(b);d=ae(p,"""
  ...
  """);app=app_path.read_text(encoding='utf-8');contract="screen:'conversation',conversationId:current?.id" in app and "screen:'canvas',conversationId:current?.id" in app and "canvasVisible&&!canvasNavigationAllowed()" in app;ok=not d['a']['canvas'] and d['a']['resourceId'] is None and d['mid']['screen']=='canvas' and d['mid']['canvas'] and d['mid']['conversationId']=='c1' and d['mid']['resourceId'] is None and d['c']['screen']=='conversation' and not d['c']['canvas'] and contract;record('1B-T04',ok,{'runtime':d,'caller_contract':contract});p.close()
 if wanted('1B-T05'):
  p=page(b);d=ae(p,"""
...
if wanted('1B-T06'):
 ...
 for path in paths:
  r=subprocess.run(['node','--check',str(path)],capture_output=True,text=True);syntax.append({'path':str(path.relative_to(root)),'code':r.returncode,'stderr':r.stderr})
 ...
 contract={'auth_user_identity':"sessionChanged(session?.user?.id||null)" in app,'conversation_screen':"screen:'conversation',conversationId:current?.id" in app,'canvas_screen':"screen:'canvas',conversationId:current?.id" in app,'controller_signal':'signal:transition.abort.signal' in ctl,'scope_cleanup':'onCleanup:transition.scope.add' in ctl,'bots_scope':'onCleanup' in bots,'scenario_signal':'signal' in bridge and 'onCleanup' in bridge}
