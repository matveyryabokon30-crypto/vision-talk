...
window.openSocket=(id,url)=>new Promise((resolve,reject)=>{
 ...
 socket.onopen=()=>resolve({url:socket.url,protocol:socket.protocol,readyState:socket.readyState,
  ...
  sendNative:Function.prototype.toString.call(WebSocket.prototype.send).includes('[native code]'),
  closeNative:Function.prototype.toString.call(WebSocket.prototype.close).includes('[native code]'),
...
async def run(args):
    ...
    async def open_socket(name):
        ...
        check('NATIVE-CONSTRUCTOR-'+name,observed=={'url':url,'protocol':'','readyState':1,'instanceof':True,
              'prototype':True,'sendNative':True,'closeNative':True,'constants':[0,1,2,3]},observed)
    ...
    with warnings.catch_warnings(record=True) as caught:
        ...
        try:
            async with async_playwright() as pw:
                ...
                await until(lambda:not net.ws_connections[first] and any(e.get('connection')==first and e.get('source')=='native.WebSocket.close' and e.get('code')==1005 for e in net.ws_events))
                check('DEFAULT-CLOSE-NO-CODE',not net.ws_channels,await page.evaluate('closes'))
                ...
                await ctx.close();result['cleanup']['context']='CLOSED';await net.drain()
                await browser.close();result['cleanup']['browser']='CLOSED'
                check('ALL-CONNECTIONS-AND-JOBS-CLOSED',not net.ws_channels and not net.ws_jobs and all(item['closed'] for item in net.ws_lifecycle.values()),net.summary())
                result['status']='PASS'
        except AssertionError as exc:result.update(status='FAIL',reason=str(exc),traceback=traceback.format_exc())
        except asyncio.CancelledError:result.update(status='ERROR',reason='GATE_DEADLINE_OR_INTERRUPTION',traceback=traceback.format_exc())
        except Exception as exc:result.update(status='ERROR',reason=str(exc),traceback=traceback.format_exc())
        finally:
            deadline.cancel()
            if collector and page and not page.is_closed():
                try:result.update(await collector.capture())
                except Exception as exc:result['capture_error']=str(exc);result['status']='ERROR'
            for name,target in [('context',ctx),('browser',browser)]:
                if target and result['cleanup'].get(name)!='CLOSED':
                    try:await asyncio.wait_for(target.close(),5);result['cleanup'][name]='CLOSED'
