...
## Текущая задача: исправить существующее приложение — 13 сентября 2026

Владелец отверг отдельный прототип и прямо поручил **вернуться к действующему Пабликусу**: исправить уход чата вверх при Enter, перенести «Боты» вниз, добавить там AI-агента, опубликовать обновление и записать результат. AI-пиктограмма по дополнительному ответу владельца **неактивна**. Больше никаких изменений. Новое поручение заменяет предыдущую стратегию этого цикла и разрешает точечный patch/deployment.

[Отчёт и критерии](docs/pablicus/fixes/2026-09-13-input-nav/REPORT.md). Причина воспроизведена в Chrome/WebKit: необрезанный textarea выходит из editor и нативная прокрутка к курсору сдвигает содержимое `#app`. Исправлены границы текстового поля и прокрутки оболочки; нижний ряд Чаты / Дела / Боты / AI-агент / Вы; существующий Bot Core route сохранён. Изменено восемь runtime-файлов, включая версию кэша доставки. Auth/Face ID, backend, данные и отдельный прототип не изменяются. Опубликовано по прежнему адресу: Pages `de297a5`, 13 сентября 00:38:51 UTC, 79/79 ресурсов совпали с проверенной версией. [Квитанция](docs/pablicus/fixes/2026-09-13-input-nav/PUBLICATION.json). Новый кэш `pablicus-shell-input-nav-20260913-r1`; черновик при обновлении сохранился. 112 локальных проверок и все четыре CI (Block 01, 1C, 2A, 2B) прошли. Физическая iPhone-проверка остаётся у владельца.

## Предыдущая задача: новая основа, отвергнутая владельцем — 13 сентября 2026

После сообщения владельца о недоступности localhost на iPhone подготовлена отдельная [HTTPS-копия прототипа](https://serene-zenith-23vt8rn.shipstatic.com). Проверены загрузка шести ресурсов, четыре раздела, боковое переключение и компактный ввод на viewport 390×844; прямая ссылка в разговор также открылась после нормализации URL хостингом. Это не физическая проверка Safari. Анонимный просмотр истекает через три дня, 16 сентября; claim-ссылка хранится только локально и передана владельцу. [Результат](docs/pablicus/design/2026-09-13-new-foundation/PHONE_PREVIEW.json), упаковщик `prototypes/pablicus-next/package_preview.py`. Production, данные и вход прежней PWA не изменены.
