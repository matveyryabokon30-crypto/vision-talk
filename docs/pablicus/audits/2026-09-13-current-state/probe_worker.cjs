...
(async()=>{
 ...
 const current=worker(source('HEAD'));await current.event('install');current.set('new');
 const currentNav=(await current.event('fetch',{request:nav}))||await current.network(nav),currentApp=await current.event('fetch',{request:app});
 result.checks.push({id:'PWA-CURRENT-QUERY-MIX',status:currentNav.generation==='new'&&currentApp.generation==='old'?'REPRODUCED':'NOT_REPRODUCED',html:currentNav,app:currentApp});
 const race=worker(source('HEAD'));race.set('old',8);await race.event('install');const generations=[...new Set([...race.stores.get(race.cacheName).values()].map(x=>x.generation))];
 ...
 const hashes=Object.fromEntries(['f5eec61','ccfdd90','f146055','c5e26c2','883003e','HEAD'].map(ref=>[ref,sha(source(ref))]));
 ...
 fs.writeFileSync('docs/pablicus/audits/2026-09-13-current-state/PWA_CURRENT_PROOF.json',JSON.stringify(result,null,2)+'\n');console.log(JSON.stringify(result,null,2));
})();
