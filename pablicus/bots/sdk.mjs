...
export class BotClient {
  constructor({baseUrl,getToken,apiKey=null,fetcher=fetch}) {
  ...
  async request(path,{method='GET',data,signal}={}){
    ...
    if(!token) throw Object.assign(new Error('Нужен вход.'),{code:'UNAUTHORIZED'});
    ...
    let body;
    try{body=await response.json();}catch{throw new Error('Сервер ботов вернул некорректный ответ.');}
    if(!response.ok) throw Object.assign(new Error(body?.error?.message||'Ошибка сервера ботов'),{status:response.status,code:body?.error?.code});
    return body;
  }
  me(){return this.request('/v1/me');}
  templates(){return this.request('/v1/templates');}
  bots(){return this.request('/v1/bots');}
