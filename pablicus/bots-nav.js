...
(function(){'use strict';
 ...
 nav.addEventListener('click',e=>{const button=e.target.closest('button[data-page]');if(!button||button===botButton)return;if(mounted){mounted=false;content.classList.remove('bots-active','factory-active');bots.reset();}},true);
 client.auth.onAuthStateChange((_event,session)=>{user=session?.user||null;if(!user&&mounted){mounted=false;content.classList.remove('bots-active','factory-active');bots.reset();}});sessionUser();
