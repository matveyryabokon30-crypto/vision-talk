/* Main-navigation adapter for Bot Core. Loaded after app.js so it can reuse Public's authenticated Supabase session indirectly through the same persisted auth session. */
(function(){'use strict';
 ...
 async function openBots(){await sessionUser();if(!user){location.reload();return;}mounted=true;title.textContent='Боты';title.hidden=true;search.hidden=true;filters.hidden=true;newChat.hidden=true;nav.querySelectorAll('button').forEach(b=>b.classList.toggle('selected',b===botButton));bots.mount(content);}
