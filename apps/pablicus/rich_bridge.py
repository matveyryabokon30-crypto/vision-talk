...
def adapt_vault(source):
    source = replace_once(source, 'indexedDB.open(this.name,1)', 'indexedDB.open(this.name,2)')
    source = replace_once(source, 'JSON.stringify({text:s.text,selection:s.selection,expanded:s.expanded,files:', 'JSON.stringify({text:s.text,blocks:s.blocks,recording:s.recording,selection:s.selection,expanded:s.expanded,files:')
    source = replace_once(source, 'return {text:await digest(new Blob([s.text])),selection:', 'return {blocks:s.blocks,recording:s.recording,text:await digest(new Blob([s.text])),selection:')
...
def adapt_chat(source):
    s = replace_once(source, 'const assets=new Map(),liveUrls=new Set();', 'let richComposer=null;\nconst assets=new Map(),liveUrls=new Set();')
    ...
    s = replace_once(s, 'function clearDraft(){', 'function clearDraft(){richComposer?.clear();')
    ...
    s = replace_once(s, 'function captureDraft(){return', 'function captureDraft(){if(richComposer)return {...richComposer.capture(),expanded:draft.expanded};return')
    ...
    s = replace_once(s, "text:m.text||'',image:false", "text:m.text||'',richBlocks:m.kind==='rich'?m.blocks:null,image:false")
    ...
    s = replace_once(s, "if(!result.deduplicated){input.value='';", "if(!result.deduplicated){richComposer?.clear();input.value='';")
...
window.PablicusChat={''')
    ...
    s = replace_once(s, "sourceMessages=messages;scopeUser=user;scopeChat=chat;", "richComposer.clear();sourceMessages=messages;scopeUser=user;scopeChat=chat;")
    ...
    s = replace_once(s, ' addFiles,fillDraft,get list()', ''' get rich(){return richComposer},
