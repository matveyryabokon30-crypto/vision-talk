...
(() => {'use strict';
 ...
 async function authenticate(session,{signal,verifiedPasskey=false}={}){
  ...
  safeSet('pablicus:'+uid+':profile',profile);$('loginPane').hidden=true;$('workspace').hidden=false;$('mainNav').hidden=false;dialogs=safeGet(cacheKey())||[];renderHome();await loadDialogs();if(user?.id!==uid)return;startInbox();pump();showPersonLink();
 }
 ...
 async function syncMessages(){if(!current||!user||syncing||opening||document.hidden||!navigator.onLine)return;syncing=true;const d=current,uid=user.id,ep=epoch;try{
  ...
  if(next!==peersRead){peersRead=next;document.querySelectorAll('#canvas [data-server-seq]').forEach(n=>{const meta=n.querySelector('.meta');if(meta&&n.classList.contains('mine'))meta.textContent=stamp(n.dataset.created)+' · '+(+n.dataset.serverSeq<=peersRead?'Прочитано':'Отправлено')+(n.dataset.edited?' · изм.':'')})}
 ...
 function renderMessage(m){const r=PablicusChatActions.effective(m.remote),row=el('article','row'+(m.mine?' mine':'')),b=el('div','bubble'),t=el('div','text');row.dataset.id=m.id;row.dataset.rev=m.revision;row.dataset.serverSeq=r.server_seq;row.dataset.created=r.created_at;row.dataset.edited=r.edited_at||'';
  ...
  b.append(t,el('div','meta',stamp(r.created_at)+(m.mine?' · '+(+r.server_seq<=peersRead?'Прочитано':'Отправлено'):'')));messageTools.decorate(row,b,m.remote);row.append(b);return row;
 }
