(() => {
'use strict';
const BUILD='gate-01.5.0-outbox',COUNT=300,LIMIT=120,OVERSCAN=560;
const $=id=>document.getElementById(id),vp=$('vp'),canvas=$('canvas'),app=$('app'),input=$('input');
let list=null,running=false,seq=COUNT,simulating=false,layoutFrame=0;
...
go(i){i=Math.max(0,Math.min(i,this.messages.length-1));this.follow=false;this.sync({id:this.messages[i].id,offset:0},false,'explicit-navigation');}
...
$('send').textContent=draft.mode==='message'?'↑':'▷';$('send').setAttribute('aria-label',draft.mode==='message'?'Сохранить в исходящие; не доставка':'Запустить демонстрацию, без ИИ');
...
async function openChat(){list?.destroy();canvas.replaceChildren();seq=COUNT;const t=performance.now();vp.style.visibility='hidden';$('startPanel').hidden=true;list=new NaturalList(dataset(1,COUNT).concat(queueMessages()));list.bottom();syncComposer({end:true},true);vp.style.visibility='visible';await frames(2);report.last_open={mount_ms:round(performance.now()-t),bottom_delta_px:round(list.bottomDistance()),last_id:list.messages.at(-1).id,dom_rows:list.nodes.size};status('Черновик сохраняется в этом браузере · без отправки');return report.last_open}
...
const selected=Array.from(files||[]),s=selection();
...
const menuRows={
attach:[['gallery','Фото и видео'],['camera','Камера'],['documents','Документы'],['modes','Помощник'],['keyboard','Клавиатура'],['clear','Очистить']],
...
if(['documents','modes','back'].includes(action)){openMenu(action==='back'?'attach':action,menuOrigin);return}
...
if(['message','assistant','task'].includes(action)){closeMenu(false);changeUI(()=>draft.mode=action);restoreSelection(savedSelection,menuWasTyping);return}
...
$('attach').onclick=e=>openMenu('attach',$('attach'),e.detail===0);$('mode').onclick=e=>openMenu('modes',$('mode'),e.detail===0);$('taskMenu').onclick=e=>openMenu('tasks',$('taskMenu'),e.detail===0);
...
async function initializeVault(){
 vault=new DraftVault.Controller({capture:captureDraft,restore:restoreDraft,paint:paintVault,lock:lockDraft});vault.store=new OutboxVault.OutboxStore();
 window.vault=vault;
