...
MOCK=r'''window.__mock={sent:[],read:0,online:true,objects:new Map()};window.supabase={createClient(){
...
return{auth:{getSession:()=>result({session:{user,access_token:'MOCK'}}),onAuthStateChange:()=>({data:{subscription:{unsubscribe(){}}}}),signInWithPassword:()=>result({session:{user}}),signOut:()=>result(null)},from:chain,
