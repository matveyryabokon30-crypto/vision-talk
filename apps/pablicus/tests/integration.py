...
rpc:async(name,args)=>{if(name==='pablicus_list_tasks_v2'||name==='pablicus_list_projects'){const expected=name==='pablicus_list_tasks_v2'?['p_view','p_query','p_today','p_cursor','p_limit','p_timezone']:['p_query','p_cursor','p_limit'];if(JSON.stringify(Object.keys(args||{}).sort())!==JSON.stringify(expected.sort()))throw Error('Unexpected workspace RPC arguments: '+name);return result({[name==='pablicus_list_tasks_v2'?'tasks':'projects']:[],next_cursor:null,total_count:0});}if(name.startsWith('my_conversations'))return result([{id:chat,title:'@qa_peer',last_message:messages.at(-1)?.body||'',last_seq:seq,last_read_seq:2,unread_count:1,last_message_at:new Date().toISOString()}]);if(name==='mark_conversation_read'){window.__mock.read=args.p_last_read_seq;return result(args.p_last_read_seq)}if(name==='start_direct_conversation'||name==='start_saved_conversation')return result(chat);if(name==='send_message'||name==='send_attachment_message'||name==='send_rich_message'){if(!window.__mock.online)return result(null,{name:'TypeError',message:'network request failed'});const old=messages.find(m=>m.client_message_id===args.p_client_message_id);if(old)return result(old);const type=name==='send_rich_message'?'rich':name==='send_message'?'text':args.p_type;const m={id:'s'+(++seq),client_message_id:args.p_client_message_id,conversation_id:chat,server_seq:seq,sender_id:user.id,type,body:name==='send_rich_message'?args.p_content.blocks.filter(b=>b.type==='text').map(b=>b.text).join('\n'):args.p_body||args.p_caption||null,attachment_path:args.p_attachment_path||null,attachment_metadata:name==='send_rich_message'?args.p_content:name==='send_attachment_message'?{name:args.p_attachment_name,mime_type:args.p_mime_type,size_bytes:args.p_size_bytes}:null,created_at:new Date().toISOString()};messages.push(m);window.__mock.sent.push(m);return result(m)}return result(null)},
