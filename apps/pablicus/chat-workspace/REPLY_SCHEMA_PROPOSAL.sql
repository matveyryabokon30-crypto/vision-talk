-- Additive reply extension for the already deployed rich-message schema.
-- Apply after SCHEMA_PROPOSAL.sql. Replaces only the private implementation;
-- existing public wrapper, grants, RLS, tables and message rows are unchanged.
BEGIN;
SET LOCAL lock_timeout = '5s';
SET LOCAL statement_timeout = '30s';

-- Fail closed if the deployed rich-message implementation is absent. Replacing
-- the existing function preserves its owner and restricted EXECUTE privileges.
DO $guard$
BEGIN
...
CREATE OR REPLACE FUNCTION pablicus_chat_private.send_rich_message(
...
DECLARE
  ...
  v_reply jsonb;
  v_reply_target public.messages;
...
BEGIN
  ...
  END IF;
  IF (p_content - ARRAY['v','blocks','reply_to']::text[]) <> '{}'::jsonb
  ...
  -- A reply contains only a reference, never a caller-supplied quotation or author.
  -- Validate after the acknowledged-retry fast path: an already sent reply keeps
  -- its identity even if its original message is subsequently removed.
  v_reply := p_content->'reply_to';
  IF v_reply IS NOT NULL AND v_reply <> 'null'::jsonb THEN
    IF jsonb_typeof(v_reply) IS DISTINCT FROM 'object'
       OR (v_reply - ARRAY['message_id','block_id']::text[]) <> '{}'::jsonb
       OR jsonb_typeof(v_reply->'message_id') IS DISTINCT FROM 'string'
       OR (v_reply->>'message_id') !~* '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$' THEN
      RAISE EXCEPTION 'invalid reply reference' USING ERRCODE='22023';
    END IF;
    IF v_reply ? 'block_id' THEN
      IF jsonb_typeof(v_reply->'block_id') IS DISTINCT FROM 'string'
         OR (v_reply->>'block_id') !~ '^[A-Za-z0-9_-]{1,128}$' THEN
        RAISE EXCEPTION 'invalid reply block' USING ERRCODE='22023';
      END IF;
    END IF;
    -- Scope the lookup even when this sender belongs to both conversations.
    -- The lock prevents deletion or block replacement before this insert commits.
    SELECT * INTO v_reply_target FROM public.messages
      WHERE id=(v_reply->>'message_id')::uuid AND conversation_id=p_conversation_id FOR SHARE;
    IF NOT FOUND THEN
      RAISE EXCEPTION 'reply target not found in conversation' USING ERRCODE='22023';
    END IF;
    IF v_reply ? 'block_id' THEN
      IF v_reply_target.type <> 'rich'
         OR jsonb_typeof(v_reply_target.attachment_metadata->'blocks') IS DISTINCT FROM 'array' THEN
        RAISE EXCEPTION 'reply block not found' USING ERRCODE='22023';
      END IF;
      IF NOT EXISTS (SELECT 1 FROM jsonb_array_elements(v_reply_target.attachment_metadata->'blocks') AS target(block)
                     WHERE target.block->>'id'=v_reply->>'block_id') THEN
        RAISE EXCEPTION 'reply block not found' USING ERRCODE='22023';
      END IF;
    END IF;
  END IF;
